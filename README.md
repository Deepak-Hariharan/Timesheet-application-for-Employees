...
1. Clone the repository: git clone https://github.com/Deepak-Hariharan/Timesheet-application-for-Employees.git<br>
2. Enter the following connection string in server\config\default.json  : "mongodb+srv://deepakhariharan1999:7PCUpiRxs0LwkAps@cluster0.nx9pp8n.mongodb.net/test"
3. We will need 2 terminals to run the application. One for the front end and one for the back end. <br>
4. Go to the client folder in one terminal from the root folder. Use the following command to do so:cd client<br>
5. Then run the following command to install all the packages that the application depends on: npm i (or) npm install
<br>
6. Run the following command to start the react app:npm run start<br>
7. Go to the server folder in another terminal from the root folder. Use the following command to do so:cd server<br>
8. Run the following command to install mongo db:
npm install mongodb<br>
9. Then run the following command to install nodemon:
npm install nodemon<br>
10. Then run the following command to start the node server:
nodemon index.js<br>
11. Once above steps are completed, the application will run on localhost:3000 and open on a new browser window<br>


## 4. TESTING
This code was tested in the following ways:

1. Different types of inputs in the form.
2. Adding to all types of calendars.
3. Different dates (past and future dates).
4. Muliple event entries to the database at the same time.

The code worked seamlessly in all the above tests.

## 5. KNOWN ISSUES
Adding an event doesn't reflect on the visual calendar that is present just below the "Add event" button. The calendar performs a get operation on the database and displays the events. 
Unable to start the application with the login page as root.
...
## 8. Get Help
If you have any issues or questions, please contact us.<br>
Deepak: deepakhariharan1999@gmail.com <br>
